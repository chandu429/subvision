...
<div class="box mb-3 px-5 py-3 grid grid-cols-5 grid-flow-row gap-2">
  <div class="col-span-2">
    <div class="text text-sm">Parachain {paraId}</div>
  </div>
  <div class="text-xs text-gray-500 col-span-3 text-right">{getTimeDiffInWord(Date.now() - new Date(createdAt).getTime())}</div>
  <div class="text-base col-span-5">
    has submit a new bid for slot {firstSlot} - {lastSlot} at <Token value={amount} />
  </div>
  <div class="text-base col-span-5 flex">
    {#if isCrowdloan }
